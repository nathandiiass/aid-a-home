  ...
  return <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl shadow-lg border-0 p-6 space-y-3">
        ...
        {!selectedCategoria ? <div className="p-4 text-center text-muted-foreground text-sm border-2 border-dashed rounded-xl">
            ...
            {availableTags.map(tag => <Badge key={tag.id} variant={selectedTags.includes(tag.tag_name) ? "default" : "outline"} className={cn("cursor-pointer transition-all py-2 px-4 text-sm", selectedTags.includes(tag.tag_name) ? "bg-[#0099ff]/[0.09] text-[#009aff] border-[#009aff]/20" : "hover:bg-[#009AFF] hover:text-white hover:border-[#009AFF]")} onClick={() => handleTagToggle(tag.tag_name)}>
                {tag.tag_name}
              </Badge>)}
          </div>}
